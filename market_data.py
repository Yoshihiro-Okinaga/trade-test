...
class MarketData:
    ...
    def calc_ref_signals(self, start_days, sma_period):
        ...
        # breakout（前日の高値を超えて引けたら +1、前日の安値を下回って引けたら -1）
        # 値の大きさに意味を持たせないイベント型。macd のゼロ交差と同じ扱いで、
        # 閾値 0.5 にしておけば「発生した日だけ」シグナルが成立する。
        # 前日高値 >= 前日安値 なので、+1 と -1 が同時に立つことはない。
        # streak / macd と同じく sma_period には依存しない。
        prev_high = ref["高値"].shift(1)
        prev_low = ref["安値"].shift(1)
        breakout = (
            (ref["ref_base"] > prev_high).astype(float)
            - (ref["ref_base"] < prev_low).astype(float)
        )
        # 初日は前日値が無い。比較演算は NaN を False にしてしまうため、
        # そのままだと「ブレイクなし(0)」として混入する。明示的に NaN へ戻す。
        breakout = breakout.where(prev_high.notna() & prev_low.notna())
        ref["ref_signal_breakout"] = breakout.shift(start_days)
        # 生シグナル(shift前)。live の pending(最終日発火)検出に使う。
        ref["ref_signal_breakout_now"] = breakout
