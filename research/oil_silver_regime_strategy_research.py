...
TRADE_OUTPUT_FILE = "oil_silver_regime_strategy_trades.csv"
SUMMARY_OUTPUT_FILE = "oil_silver_regime_strategy_summary.csv"
COMPARISON_OUTPUT_FILE = (
    "oil_silver_regime_strategy_comparison.csv"
)
ROUND_DIGITS = 9
...
class RegimeStrategyConfig:
    ...
    def from_config_data(
        ...
        raw = config_data.get(
            "oil_silver_regime_strategy_research",
            {},
        )
        ...
        periods = tuple(
            _parse_period(
                value,
                "oil_silver_regime_strategy_research.periods",
            )
        ...
        try:
            ...
            raise ValueError(
                "oil_silver_regime_strategy_research.signal_type "
                "が不正です。"
            ) from exc
...
def validate_backtest_environment(
    ...
    """代表戦略を選んだ時点の本体条件が変わっていないか確認する。"""
    if backtest_config.no_overlap is not True:
        raise ValueError(
            "no_overlap は true に固定してください。"
        )

    if backtest_config.use_excess_return != [False]:
        raise ValueError(
            "use_excess_return は [false] "
            "に固定してください。"
        )

    if backtest_config.filter_signal_type:
        raise ValueError(
            "filter_signal_type は空文字にしてください。"
        )

    if not math.isclose(
        float(backtest_config.extra_cost_pct),
        0.0,
        abs_tol=1e-12,
    ):
        raise ValueError(
    ...
    if (
        backtest_config.trade_code_type
        != TradeCodeType.ALL
    ):
        raise ValueError(
    ...
    oil_cost = backtest_config.cost_of(
        FROZEN_TARGET
    )
    if not math.isclose(
        oil_cost,
    ...
    oil_swap = backtest_config.swap_of(
        FROZEN_TARGET
    )
    if not math.isclose(
        oil_swap,
...
def trim_market_data_end_year(
    ...
    """2021+を計算対象へ入れないため、MarketData本体を2020末で切る。"""
...
def build_task_caches(
    ...
    # ファイルに2021+が入っていても、今回の研究計算には渡さない。
...
def run(
    ...
    research_config = (
        RegimeStrategyConfig.from_config_data(
            config_data
        )
    )
