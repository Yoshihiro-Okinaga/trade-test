...
# COPPER/OIL 2.5σ Pair専用のdevelopment出力名にする。
TRADE_OUTPUT_FILE = "copper_oil_pair_final_oos_trades.csv"
SUMMARY_OUTPUT_FILE = "copper_oil_pair_final_oos_summary.csv"
PORTFOLIO_OUTPUT_FILE = "copper_oil_pair_final_oos_portfolio.csv"
PORTFOLIO_SUMMARY_OUTPUT_FILE = (
    "copper_oil_pair_final_oos_portfolio_summary.csv"
)
VERDICT_OUTPUT_FILE = (
    "copper_oil_pair_final_oos_verdict.csv"
)

# COPPER/OIL 2.5σ Pairのstrategy-specific final OOS条件。
# 2021-2025を一度だけ評価し、2026年は読まない。
...
FROZEN_PAIR_ALLOCATIONS = (
    ("COPPER_USD", "OIL_USD", 1.0),
)
FROZEN_RETURN_BASIS = "close_price_only"

# developmentは12 tradesだった希少イベント戦略。
# 8未満ならサンプル不足として判定保留にする。
FROZEN_MIN_CLOSED_TRADES = 8
FROZEN_MIN_BASELINE_AVERAGE_PCT = 0.0
FROZEN_MIN_BASELINE_MEDIAN_PCT = 0.0
FROZEN_MIN_BASELINE_WIN_RATE = 50.0
FROZEN_MIN_STRESS_AVERAGE_PCT = 0.0
FROZEN_MIN_BASELINE_TERMINAL_RETURN_PCT = 0.0
FROZEN_MIN_STRESS_TERMINAL_RETURN_PCT = 0.0
...
class PairStrategyConfig:
    ...
    def from_config_data(cls, config_data: dict):
        ...
        hedge_fit_period = _parse_period(
            ...
            "copper_oil_pair_final_oos.hedge_fit_period",
        )
    ...
    def validate_final_oos_freeze(self):
        ...
        for name, actual, expected in frozen_scalars:
            if actual != expected:
                raise ValueError(
                    f"COPPER/OIL final OOSの {name} は {expected!r} "
                    f"に固定しています。実際: {actual!r}"
                )

        if self.pair_allocations != FROZEN_PAIR_ALLOCATIONS:
            raise ValueError(
                "COPPER/OIL final OOSの pair_allocations は100%に固定しています。"
            )
...
def _parse_pairs(
    ...
    for value in values:
        if len(value) != 2:
            raise ValueError(
                "copper_oil_pair_final_oos.pairs の各要素は"
                " [銘柄A, 銘柄B] にしてください。"
            )
    ...
    if not pairs:
        raise ValueError(
            "copper_oil_pair_final_oos.pairs を"
            "1組以上指定してください。"
        )
...
def validate_verdict_config(
    config_data: dict,
):
    """final OOSを見る前に固定した合否基準を確認する。"""
    section = config_data.get(
        "copper_oil_pair_final_oos",
        {},
    )
    ...
    expected = {
        "min_closed_trades": FROZEN_MIN_CLOSED_TRADES,
        ...
        "min_stress_average_pct": (
            FROZEN_MIN_STRESS_AVERAGE_PCT
        ),
        "min_baseline_terminal_return_pct": (
            FROZEN_MIN_BASELINE_TERMINAL_RETURN_PCT
        ),
        "min_stress_terminal_return_pct": (
            FROZEN_MIN_STRESS_TERMINAL_RETURN_PCT
        ),
    }
    ...
    for name, expected_value in expected.items():
        ...
        if actual != expected_value:
            raise ValueError(
                f"COPPER/OIL final OOSの {name} は "
                f"{expected_value!r} に固定しています。"
                f"実際: {actual!r}"
            )
...
def build_verdict(
    ...
    if len(rows) != 1:
        raise ValueError(
            "portfolio_all summaryを一意に取得できません。"
        )
    ...
    checks = {
        ...
        "stress_average_positive": (
            row["stress_average_pct"]
            > FROZEN_MIN_STRESS_AVERAGE_PCT
        ),
        "baseline_terminal_positive": (
            row["baseline_terminal_return_pct"]
            > FROZEN_MIN_BASELINE_TERMINAL_RETURN_PCT
        ),
        "stress_terminal_positive": (
            row["stress_terminal_return_pct"]
            > FROZEN_MIN_STRESS_TERMINAL_RETURN_PCT
        ),
    }
    ...
    if not checks["enough_trades"]:
        ...
        not checks["baseline_average_positive"]
        or not checks["baseline_terminal_positive"]
    ):
        verdict = "FAIL"
    ...
    result = {
        "verdict": verdict,
        "period_start_year": FROZEN_EVALUATION_PERIOD[0],
        "period_end_year": FROZEN_EVALUATION_PERIOD[1],
        "trade_count": int(
            row["baseline_trade_count"]
        ),
        ...
        "baseline_t_value": (
            row["baseline_t_value"]
        ),
        "baseline_terminal_return_pct": (
            row["baseline_terminal_return_pct"]
        ),
        "stress_average_pct": (
            row["stress_average_pct"]
...
def run(
    ...
    validate_verdict_config(
        config_data
    )

    print("=== COPPER/OIL 2.5σ Pair FINAL OOS ===")
    print(
        "評価期間: "
    ...
    print(
        "Strategy-specific FINAL OOS: 2021-2025を一度だけ評価します。"
    )
    print(
        "2026年は今回読みません。将来のforward期間として残します。"
    )
    print(
        "Return basis: COPPER/OIL終値の価格リターン。"
    )
    print("固定コスト:")
    for scenario in config.cost_scenarios:
        primary_mark = (
    ...
    print(
        "Portfolio: COPPER/OIL単独Pairへ100%固定配分。"
    )
    ...
    print(
        verdict_frame.to_string(
            index=False,
        )
    )
    print(
        "\n最終OOSを実行しました。"
        "このStepでは2021-2025のPair損益を見ません。"
        "2016-2020の結果を確認してから次へ進みます。"
    )
