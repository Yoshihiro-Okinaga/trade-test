...
TRADE_OUTPUT_FILE = "copper_oil_pair_strategy_trades.csv"
SUMMARY_OUTPUT_FILE = "copper_oil_pair_strategy_summary.csv"
PORTFOLIO_OUTPUT_FILE = "copper_oil_pair_strategy_portfolio.csv"
PORTFOLIO_SUMMARY_OUTPUT_FILE = (
    "copper_oil_pair_strategy_portfolio_summary.csv"
)

# COPPER/OIL 2.5σ仮説を実売買化するStep 1の固定条件。
# 2021+のPair損益を見る前に、2016-2020だけで確認する。
...
class PairStrategyConfig:
    ...
    def from_config_data(cls, config_data: dict):
        section = config_data.get(
            "copper_oil_pair_strategy_research",
            {},
        )

        evaluation_period = _parse_period(
            section.get("period", [2016, 2020]),
            "copper_oil_pair_strategy_research.period",
        )
        hedge_fit_period = _parse_period(
            ...
            "copper_oil_pair_strategy_research.hedge_fit_period",
        )
    ...
    def validate_final_oos_freeze(self):
        ...
        for name, actual, expected in frozen_scalars:
            if actual != expected:
                raise ValueError(
                    f"COPPER/OIL Step 1の {name} は {expected!r} "
                    f"に固定しています。実際: {actual!r}"
                )

        if self.pair_allocations != FROZEN_PAIR_ALLOCATIONS:
            raise ValueError(
                "COPPER/OIL Step 1の pair_allocations は100%に固定しています。"
            )
...
def _parse_pairs(
    ...
    for value in values:
        if len(value) != 2:
            raise ValueError(
                "copper_oil_pair_strategy_research.pairs の各要素は"
                " [銘柄A, 銘柄B] にしてください。"
            )
    ...
    if not pairs:
        raise ValueError(
            "copper_oil_pair_strategy_research.pairs を"
            "1組以上指定してください。"
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "COPPER/OIL 2.5σ大乖離Pairを、"
            "固定した単純売買ルールで2016-2020検証する。"
        )
    )
    parser.add_argument(
        "--config",
